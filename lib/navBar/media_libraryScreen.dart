...
class MediaLibraryProvider with ChangeNotifier {
  ...
  Future<List<MovieCard>> fetchMediaLibrary() async {
    final response = await http.get(
      Uri.parse('$_apiBaseUrl/movie/popular?api_key=$apiKey'),
    );
...
class ApiService {
  static const String _apiKey = '7e2888cb00457031860b026f0415280c';
  static const String _apiBaseUrl = 'https://api.themoviedb.org/3';
...
class Media_libraryScreen with ChangeNotifier {
  List<MovieCard> _mediaLibrary = [];
  ...
  Future<List<MovieCard>> fetchMediaLibrary() async {
    try {
      final List<dynamic> data = await ApiService.fetchPopularMovies();
      _mediaLibrary = data.map((movie) => MovieCard.fromJson(movie)).toList();
      notifyListeners();
      return _mediaLibrary;
    } catch (e) {
